...
import { requestDeposit } from '@/lib/actions/dashboard'
...
export default function DepositModal({ open, onClose }: DepositModalProps) {
  const [state, formAction, isPending] = useActionState(requestDeposit, null)

  return (
    <Modal open={open} onClose={onClose} title="Request Deposit">
      {state?.success ? (
        <div className="flex flex-col items-center gap-4 py-4 text-center">
          ...
          <div>
            <p className="text-text-primary font-semibold text-base">Deposit request submitted</p>
            <p className="text-text-muted text-sm mt-1">Pending admin approval. You'll be notified once processed.</p>
          </div>
        ...
        <form action={formAction} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            />
            <p className="text-text-muted text-xs">Minimum deposit: $10.00</p>
          </div>
          ...
          >
            {isPending ? 'Submitting…' : 'Request Deposit'}
          </Button>
        </form>
      )}
    </Modal>
